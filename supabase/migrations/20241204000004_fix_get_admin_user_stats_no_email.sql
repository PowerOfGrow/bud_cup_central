-- Fix v4: Version sans email pour isoler le problème de auth.users
-- Si cette version fonctionne, le problème vient de l'accès à auth.users
...
begin
  ...
  select
    p.id,
    p.display_name,
    ''::text as email,  -- Email vide pour tester
    p.role,
    p.created_at,
    p.is_verified,
...
comment on function public.get_admin_user_stats is 'Statistiques détaillées par utilisateur pour la page Admin - Version sans email pour test';
